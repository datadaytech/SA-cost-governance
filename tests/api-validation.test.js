/**
 * API Validation Tests for SA-cost-governance
...
describe('Splunk API Validation', () => {
    ...
    describe('App Installation', () => {
        test('SA-cost-governance app should be installed', async () => {
            const response = await splunkRequest('/services/apps/local/SA-cost-governance');
            expect(response.status).toBe(200);
        }, 30000);
    ...
    describe('Lookups', () => {
        test('governance_cache_sid_lookup should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/transforms/lookups/governance_cache_sid_lookup');
            expect(response.status).toBe(200);
        }, 30000);

        test('flagged_searches_lookup should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/transforms/lookups/flagged_searches_lookup');
            expect(response.status).toBe(200);
        }, 30000);

        test('governance_settings_lookup should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/transforms/lookups/governance_settings_lookup');
            expect(response.status).toBe(200);
        }, 30000);
    });
    ...
    describe('Macros', () => {
        test('analyze_search_costs macro should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/admin/macros/analyze_search_costs');
            expect(response.status).toBe(200);
        }, 30000);

        test('load_cached_cost_analysis macro should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/admin/macros/load_cached_cost_analysis');
            expect(response.status).toBe(200);
        }, 30000);
    });
    ...
    describe('Dashboards', () => {
        test('cost_analysis dashboard should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/ui/views/cost_analysis');
            expect(response.status).toBe(200);
        }, 30000);

        test('governance_settings dashboard should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/ui/views/governance_settings');
            expect(response.status).toBe(200);
        }, 30000);

        test('scheduled_search_governance dashboard should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/ui/views/scheduled_search_governance');
            expect(response.status).toBe(200);
        }, 30000);

        test('dashboard_governance dashboard should exist', async () => {
            const response = await splunkRequest('/servicesNS/admin/SA-cost-governance/data/ui/views/dashboard_governance');
            expect(response.status).toBe(200);
        }, 30000);
    });
