...
The SA-cost-governance app uses **CSV-based lookup files** for data storage. This approach provides:
...
CSV files are stored in: `$SPLUNK_HOME/etc/apps/SA-cost-governance/lookups/`

#### View files directly:

```bash
# On Splunk server
cat $SPLUNK_HOME/etc/apps/SA-cost-governance/lookups/flagged_searches.csv
cat $SPLUNK_HOME/etc/apps/SA-cost-governance/lookups/governance_audit_log.csv
```

#### Using Docker:

```bash
docker exec splunk-dev cat /opt/splunk/etc/apps/SA-cost-governance/lookups/flagged_searches.csv
docker exec splunk-dev cat /opt/splunk/etc/apps/SA-cost-governance/lookups/governance_audit_log.csv
```
...
tar -czvf governance_lookups_backup_$(date +%Y%m%d).tar.gz \
    $SPLUNK_HOME/etc/apps/SA-cost-governance/lookups/*.csv
```
...
tar -xzvf governance_lookups_backup_YYYYMMDD.tar.gz \
    -C $SPLUNK_HOME/etc/apps/SA-cost-governance/lookups/
```
...
1. File exists: `ls $SPLUNK_HOME/etc/apps/SA-cost-governance/lookups/`
