/*
 * Cost Governance Dashboard JavaScript
 * Handles UI interactions for flagging, notifications, and enforcement actions
 *
 * SA-cost-governance v2.0.0
 *
 * Changes in v2.0.0:
 * - US-01: Removed 'Selected Search Details' panel, keeping only 'All Scheduled Searches'
 * - US-02: New icon logic - ⚡suspicious, 🚩flagged, 🔔notified, 🔴disabled (overrides all)
 * - US-03: Days Left - blank for OK/Suspicious/Disabled, 'Awaiting Notification' for Flagged, countdown for Notified
 * - US-04: Reason column shows suspicious_reason and flag_reason in main table
 * - US-05: Timer auto-starts when status → Notified (7 day default)
 * - US-06: Auto-email on Notified with mailto link and search editor URL
 * - US-07: Auto-email on Extend with new deadline and remaining time
 * - US-08: Extend UX with 📅 icon, preset buttons (+3, +7, +14, +30 days), and custom date picker
 * - US-09: Clear flag when → OK or → Disabled, preserving verbose audit history
 * - US-10: Verbose audit logging with all fields for compliance tracking
 */
...
require([
    ...
    console.log("SA-cost-governance: Starting initialization...");
    console.log("SA-cost-governance: jQuery version:", $.fn.jquery);
    console.log("SA-cost-governance: mvc available:", typeof mvc !== 'undefined');
    ...
    // Helper to decode HTML entities (reverse of escapeHtml)
    ...
    // === US-06: Auto-email on Notified ===
    // Generate and open mailto link for governance notification
    function triggerNotificationEmail(searchName, owner, app, deadlineDays) {
        var emailDomain = CONFIG.emailDomain || 'example.com';
        var toEmail = owner + '@' + emailDomain;

        // Build search editor URL
        var localePrefix = '';
        var pathParts = window.location.pathname.split('/');
        if (pathParts.length > 1 && pathParts[1].match(/^[a-z]{2}(-[A-Z]{2})?$/)) {
            localePrefix = '/' + pathParts[1];
        }
        var editUrl = window.location.origin + localePrefix + '/manager/' + app + '/saved/searches?search=' + encodeURIComponent(searchName);

        var subject = 'Splunk Governance Alert: ' + searchName + ' requires attention';
        var body = 'Hello ' + owner + ',\n\n' +
            'Your scheduled search "' + searchName + '" has been flagged for review by the Splunk Governance team.\n\n' +
            'DEADLINE: You have ' + deadlineDays + ' days to remediate this issue.\n\n' +
            'ACTION REQUIRED:\n' +
            '1. Review the search and its resource usage\n' +
            '2. Optimize or disable the search if necessary\n' +
        ...
        showToast('📧 Email drafted for ' + owner);
    }

    // === US-07: Auto-email on Extend ===
    // Generate and open mailto link for deadline extension notification
    function triggerExtendEmail(searchName, owner, app, newDeadlineDate, remainingDays) {
        var emailDomain = CONFIG.emailDomain || 'example.com';
        var toEmail = owner + '@' + emailDomain;
        ...
        var subject = 'Splunk Governance Update: Deadline Extended for ' + searchName;
        var body = 'Hello ' + owner + ',\n\n' +
            'The remediation deadline for your scheduled search "' + searchName + '" has been extended.\n\n' +
            'NEW DEADLINE: ' + newDeadlineDate + '\n' +
            'TIME REMAINING: ' + remainingDays + ' days\n\n' +
            'Please ensure you complete the required remediation before the new deadline.\n\n' +
            'EDIT SEARCH:\n' +
            editUrl + '\n\n' +
            'Best regards,\n' +
            'Splunk Governance Team';
        ...
        showToast('📧 Extension email drafted for ' + owner);
    }
    ...
    // ============================================================================
    // EVENT-SOURCING: Write state change events via saved search dispatch
    // This bypasses the outputlookup permission issue by dispatching a saved search
    // that runs in the splunkd context with full permissions.
    // The event is written to governance_events index (source of truth)
    // A scheduled search rebuilds flagged_searches.csv cache every 5 minutes
    // ============================================================================
    function writeStateEvent(eventData, callback) {
        var now = Math.floor(Date.now() / 1000);

        // Use REST API endpoint to update lookup directly
        // This bypasses the outputlookup permission issues
        var isOkStatus = eventData.new_status === 'ok';
        var lookupName = isOkStatus ? 'ok_searches.csv' : 'flagged_searches.csv';

        console.log("writeStateEvent: Using REST API to update lookup:", lookupName);

        // Prepare data for REST endpoint
        var postData = {
            lookup_name: lookupName,
            search_name: eventData.search_name || '',
            search_owner: eventData.search_owner || 'unknown',
            search_app: eventData.search_app || 'unknown',
            flagged_by: eventData.flagged_by || eventData.actor || currentUser,
            flagged_time: String(eventData.flagged_time || now),
            notification_sent: String(eventData.notification_sent !== undefined ? eventData.notification_sent : 0),
            notification_time: String(eventData.notification_time !== undefined ? eventData.notification_time : 0),
            remediation_deadline: String(eventData.remediation_deadline !== undefined ? eventData.remediation_deadline : 0),
            status: eventData.new_status || 'pending',
        ...
        $.ajax({
            url: '/en-US/splunkd/__raw/servicesNS/nobody/SA-cost-governance/admin/update_lookup/' + encodeURIComponent(eventData.search_name) + '?output_mode=json',
            type: 'POST',
            data: postData,
            dataType: 'json',
            success: function(response) {
                console.log("writeStateEvent: REST API update successful", response);
                if (callback) callback(null, { success: true });
            },
            error: function(xhr, status, error) {
                console.error("writeStateEvent: REST API error:", status, error);
                console.log("writeStateEvent: Trying fallback approach...");
                // Fallback to saved search approach
                fallbackToSavedSearch(eventData, callback);
            }
        });
    }

    // Fallback: Try saved search if REST API fails
    function fallbackToSavedSearch(eventData, callback) {
        var now = Math.floor(Date.now() / 1000);
        var isOkStatus = eventData.new_status === 'ok';
        var savedSearchName = isOkStatus ? "Governance - Write OK Status" : "Governance - Write State Event";

        var params = {
            'args.event_time': String(eventData.event_time || now),
            'args.search_name': eventData.search_name || '',
            'args.search_owner': eventData.search_owner || 'unknown',
            'args.search_app': eventData.search_app || 'unknown',
            'args.old_status': eventData.old_status || 'unknown',
            'args.new_status': eventData.new_status || 'pending',
            'args.actor': eventData.actor || currentUser,
            'args.reason': eventData.reason || 'Manual action',
            'args.flagged_by': eventData.flagged_by !== undefined ? eventData.flagged_by : '',
            'args.flagged_time': eventData.flagged_time !== undefined ? String(eventData.flagged_time) : '',
            'args.notification_sent': eventData.notification_sent !== undefined ? String(eventData.notification_sent) : '',
            'args.notification_time': eventData.notification_time !== undefined ? String(eventData.notification_time) : '',
            'args.remediation_deadline': eventData.remediation_deadline !== undefined ? String(eventData.remediation_deadline) : '',
            'args.notes': eventData.notes !== undefined ? eventData.notes : ''
        };

        console.log("fallbackToSavedSearch: Trying saved search:", savedSearchName);

        var service = mvc.createService();
        service.savedSearches().fetch(function(err, savedSearches) {
            if (err) {
                console.error("fallbackToSavedSearch: Error:", err);
                fallbackToDirectSearch(eventData, callback);
                return;
            }

            var savedSearch = savedSearches.item(savedSearchName);
            if (!savedSearch) {
                fallbackToDirectSearch(eventData, callback);
                return;
            }

            savedSearch.dispatch(params, function(err, job) {
                if (err) {
                    fallbackToDirectSearch(eventData, callback);
                    return;
                }

                job.track({}, function(err) {
                    if (err) {
                        console.error("fallbackToSavedSearch: Job failed:", err);
                        if (callback) callback(err, null);
                    } else {
                        console.log("fallbackToSavedSearch: Job completed successfully");
                        if (callback) callback(null, { success: true, sid: job.sid });
                    }
                });
            });
        });
    ...
    // Fallback: Use REST API to update lookup directly
    function fallbackToDirectSearch(eventData, callback) {
        var now = Math.floor(Date.now() / 1000);
        console.log("fallbackToDirectSearch: Using REST API approach to update lookup");

        // Step 1: Read current lookup data
        var readQuery = '| inputlookup flagged_searches_lookup | outputcsv';
        var service = mvc.createService();
        ...
        service.search(
            ...
            function(err, job) {
                if (err) {
                    console.error("fallbackToDirectSearch: Error reading lookup:", err);
                    showToast('Action queued - will be applied within 5 minutes');
                    if (callback) callback(err, null);
                    return;
                }
                ...
                job.track({}, function(err) {
                    if (err) {
                        console.error("fallbackToDirectSearch: Job tracking error:", err);
                        showToast('Action queued - will be applied within 5 minutes');
                        if (callback) callback(err, null);
                        return;
                    }

                    job.results({count: 0}, function(err, results) {
                        if (err) {
                            console.error("fallbackToDirectSearch: Error getting results:", err);
                            showToast('Action queued - will be applied within 5 minutes');
                            if (callback) callback(err, null);
                            return;
                        }

                        // Step 2: Build updated lookup data
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            ...
                            // Update existing record or keep as-is
                            if (row.search_name === eventData.search_name) {
                                foundExisting = true;
                                row.status = eventData.new_status || row.status;
                                row.search_owner = eventData.search_owner || row.search_owner || 'unknown';
                                row.search_app = eventData.search_app || row.search_app || 'unknown';
                                row.flagged_by = eventData.flagged_by || row.flagged_by || currentUser;
                                row.flagged_time = eventData.flagged_time || row.flagged_time || now;
                                row.notification_sent = eventData.notification_sent !== undefined ? eventData.notification_sent : row.notification_sent;
                                row.notification_time = eventData.notification_time !== undefined ? eventData.notification_time : row.notification_time;
                                row.remediation_deadline = eventData.remediation_deadline !== undefined ? eventData.remediation_deadline : row.remediation_deadline;
                                row.reason = eventData.reason || row.reason || '';
                                row.notes = eventData.notes !== undefined ? eventData.notes : (row.notes || '');
                            }

                            // Skip records with status 'ok' (they go to different lookup)
                            if (row.status !== 'ok') {
                                lookupData.push(row);
                            }
                        }
                        ...
                        if (!foundExisting && eventData.new_status !== 'ok') {
                            lookupData.push({
                                search_name: eventData.search_name || '',
                                search_owner: eventData.search_owner || 'unknown',
                                search_app: eventData.search_app || 'unknown',
                                flagged_by: eventData.flagged_by || currentUser,
                                flagged_time: eventData.flagged_time || now,
                                notification_sent: eventData.notification_sent || 0,
                                notification_time: eventData.notification_time || 0,
                                remediation_deadline: eventData.remediation_deadline || 0,
                                status: eventData.new_status || 'pending',
                                reason: eventData.reason || '',
                                notes: eventData.notes || ''
                            });
                        ...
                        var csvHeader = 'search_name,search_owner,search_app,flagged_by,flagged_time,notification_sent,notification_time,remediation_deadline,status,reason,notes';
                        var csvRows = [csvHeader];
                        for (var k = 0; k < lookupData.length; k++) {
                            var r = lookupData[k];
                            csvRows.push([
                                escapeCSV(r.search_name),
                                escapeCSV(r.search_owner),
                                escapeCSV(r.search_app),
                                escapeCSV(r.flagged_by),
                                r.flagged_time || '',
                                r.notification_sent || '0',
                                r.notification_time || '0',
                                r.remediation_deadline || '0',
                                escapeCSV(r.status),
                                escapeCSV(r.reason),
                                escapeCSV(r.notes)
                            ].join(','));
                        ...
                        // Step 4: Stage file and upload via REST API
                        stageLookupAndUpload('flagged_searches.csv', csvContent, function(uploadErr) {
                            if (uploadErr) {
                                console.error("fallbackToDirectSearch: Upload failed:", uploadErr);
                                showToast('Action queued - will be applied within 5 minutes');
                            } else {
                                console.log("fallbackToDirectSearch: Lookup updated successfully");
                            }
                            if (callback) callback(uploadErr, uploadErr ? null : {success: true});
                        });
                    });
                });
            }
    ...
    // Stage lookup file and upload via REST API
    function stageLookupAndUpload(filename, csvContent, callback) {
        // Step 1: Write staging file using a scripted input or echo command via search
        var stagingFilename = filename.replace('.csv', '_staging.csv');
        var stagingPath = '/opt/splunk/var/run/splunk/lookup_tmp/' + stagingFilename;

        // Create a search that writes the CSV content to a file
        // Using printf with base64 decode to handle special characters
        var base64Content = btoa(unescape(encodeURIComponent(csvContent)));
        var writeQuery = '| makeresults | eval cmd="echo \'' + base64Content + '\' | base64 -d > ' + stagingPath + '" | script';

        // Alternative: Use REST API to directly update the lookup
        // This works by reading current file, modifying, and writing back
        var service = mvc.createService();

        // Try direct file POST to lookup endpoint
        console.log("stageLookupAndUpload: Attempting lookup update for", filename);

        // Use XMLHttpRequest for better control over the request
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '/en-US/splunkd/__raw/servicesNS/nobody/SA-cost-governance/data/lookup-table-files/' + filename + '?output_mode=json', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');

        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                if (xhr.status >= 200 && xhr.status < 300) {
                    console.log("stageLookupAndUpload: Upload successful");
                    if (callback) callback(null);
                } else {
                    console.error("stageLookupAndUpload: Upload failed with status", xhr.status, xhr.responseText);
                    // Fall back to optimistic UI update
                    if (callback) callback(xhr.responseText);
                }
            }
        };

        // Try sending the CSV content directly
        // Note: This may not work if the server requires a file path
        xhr.send('eai:data=' + encodeURIComponent(stagingPath));
    }

    // Alternative: Write staging file using script search (for environments where REST fails)
    function writeToStagingFile(filename, csvContent, callback) {
        var stagingPath = '/opt/splunk/var/run/splunk/lookup_tmp/' + filename;
        var escapedContent = csvContent.replace(/'/g, "'\\''");

        // Use a search with script to write the file
        var searchQuery = "| makeresults | eval _raw=\"" + escapedContent.replace(/"/g, '\\"').replace(/\n/g, '\\n') + "\" | outputcsv " + filename;

        var service = mvc.createService();
        service.search(searchQuery, {earliest_time: '-1h', latest_time: 'now'}, function(err, job) {
            if (err) {
                console.error("writeToStagingFile: Error:", err);
                if (callback) callback(err);
                return;
            }
            job.track({}, function(trackErr) {
                if (callback) callback(trackErr);
            });
        });
    }

    // Helper function to update state and refresh UI
    function updateStateAndRefresh(searchName, newStatus, eventData, callback) {
        writeStateEvent(eventData, function(err, result) {
            if (err) {
                console.warn("State event write may have failed, but UI will update optimistically");
            }
    ...
    window.writeStateEvent = writeStateEvent;
    window.updateStateAndRefresh = updateStateAndRefresh;
    ...
    // Log action to audit log with comprehensive details
    // @param action - The action being performed (flagged, notified, extended, etc.)
    // @param searchName - The name of the search being acted upon
    // @param details - Human-readable description of the action
    // @param options - Optional object with additional fields:
    //   - search_owner, search_app, old_status, new_status
    //   - old_deadline, new_deadline, reason, notes, extension_days
    function logAction(action, searchName, details, options) {
        var now = Math.floor(Date.now() / 1000);
        var opts = options || {};

        // Build verbose log entry with all available fields
        var searchQuery = '| makeresults ' +
            '| eval timestamp=' + now +
            ', action="' + escapeString(action) + '"' +
            ', search_name="' + escapeString(searchName) + '"' +
            ', search_owner="' + escapeString(opts.search_owner || '') + '"' +
            ', search_app="' + escapeString(opts.search_app || '') + '"' +
            ', performed_by="' + escapeString(currentUser) + '"' +
            ', session_user="' + escapeString(currentUser) + '"' +
            ', old_status="' + escapeString(opts.old_status || '') + '"' +
            ', new_status="' + escapeString(opts.new_status || '') + '"' +
            ', old_deadline="' + escapeString(opts.old_deadline || '') + '"' +
            ', new_deadline="' + escapeString(opts.new_deadline || '') + '"' +
            ', extension_days="' + escapeString(opts.extension_days || '') + '"' +
            ', reason="' + escapeString(opts.reason || '') + '"' +
            ', notes="' + escapeString(opts.notes || '') + '"' +
            ', details="' + escapeString(details) + '"' +
            ', source_url="' + escapeString(window.location.href) + '"' +
            '| table timestamp, action, search_name, search_owner, search_app, performed_by, session_user, old_status, new_status, old_deadline, new_deadline, extension_days, reason, notes, details, source_url' +
        ...
        runSearch(searchQuery, function(err, results) {
            if (err) {
                ...
                console.log("Audit log entry created:", action, searchName, details);
            }
        });
    }
    ...
    window.unflagSearch = function() {
        ...
        // Use writeStateEvent for each search (consistent with other actions)
        searches.forEach(function(s) {
            var eventData = {
                event_time: now,
                search_name: s.searchName,
                search_owner: s.owner || 'unknown',
                search_app: s.app || 'unknown',
                old_status: s.status || 'flagged',
                new_status: 'resolved',
                actor: currentUser,
                reason: 'Marked as resolved by ' + currentUser
            };
            ...
            writeStateEvent(eventData, function(err, result) {
                ...
                if (err) {
                    errors++;
                    console.error('Error unflagging search:', s.searchName, err);
                } else {
                    logAction("unflagged", s.searchName, "Marked as resolved by " + currentUser);
                }

                // When all searches are processed
                if (completed === searches.length) {
                    if (errors > 0) {
                        showToast('Unflagged with ' + errors + ' error(s)');
                    } else {
                        var resultMsg = searches.length === 1
        ...
        '<div class="metric-popup-overlay" id="metricPopupOverlay">' +
            '<div class="metric-popup" style="max-width: 950px; width: 90%;">' +
                ...
                '<div class="metric-popup-footer" style="display: flex; gap: 10px; justify-content: flex-end; padding: 15px; border-top: 1px solid rgba(255,255,255,0.1); flex-wrap: wrap;">' +
                    ...
                    '<button class="btn btn-primary" style="display: none;" id="metricPopupExtend">Extend Deadline</button>' +
    ...
    function initModals() {
        ...
        $(document).on('click', '#okConfirmModalSave', function() {
            ...
            // ============================================================================
            // EVENT-SOURCING: Write OK (whitelist) state change via saved search dispatch
            // This creates an immutable event in the governance_events index with status='ok'
            // A scheduled search rebuilds the ok_searches cache from these events
            // ============================================================================
            var now = Math.floor(Date.now() / 1000);
            var eventData = {
                event_time: now,
                search_name: currentOkSearch.name,
                search_owner: currentOkSearch.owner,
                search_app: currentOkSearch.app,
                old_status: 'suspicious',
                new_status: 'ok',
                actor: currentUser,
                flagged_by: currentUser, // Using as approved_by
                flagged_time: now,        // Using as approved_time
                notification_sent: 0,
                notification_time: 0,
                remediation_deadline: 0,
                reason: 'Whitelisted as OK',
                notes: note
            };
            ...
            writeStateEvent(eventData, function(err, result) {
                if (err) {
                    ...
                    showToast('Error marking search as OK - will sync within 5 minutes');
                }
        ...
        $(document).on('click', '.status-dropdown-wrapper', function(e) {
            ...
            if (isSuspicious) {
                ...
                statuses = [
                    { value: 'pending', label: 'Flag for Review', color: '#f1813f' },
        ...
        $(document).on('click', '.status-option', function(e) {
            ...
            // ============================================================================
            // EVENT-SOURCING: Write state change via saved search dispatch
            // This bypasses outputlookup permission issues by writing to governance_events index
            // A scheduled search rebuilds the CSV cache every 5 minutes
            // ============================================================================
            var now = Math.floor(Date.now() / 1000);
            var eventData = {
                event_time: now,
                ...
                actor: currentUser,
                reason: 'Manual status change via dropdown'
            };

            if ((isSuspicious || isUnflagged) && newStatus === 'pending') {
                // For suspicious (unflagged) or OK (resolved) searches, we need to CREATE entry
                // NOTE: deadline is 0 until admin sends notification (then it gets set)
                eventData.new_status = 'pending';
                eventData.flagged_by = currentUser;
                eventData.flagged_time = now;
                eventData.notification_sent = 0;
                eventData.notification_time = 0;
                eventData.remediation_deadline = 0;
                eventData.reason = 'Suspicious pattern detected';
                eventData.notes = '';
            } else if (newStatus === 'notified') {
                // === US-05: Timer auto-start when status → Notified ===
                // When changing to "notified", set the 7-day deadline and start the timer
                var remediationDays = CONFIG.remediationDays || 7;
                var remediationDeadline = now + (remediationDays * 86400);

                eventData.new_status = 'notified';
                eventData.notification_sent = 1;
                eventData.notification_time = now;
                eventData.remediation_deadline = remediationDeadline;

                // If going directly from OK/Suspicious to Notified, also set flagged fields
                if (isSuspicious || isUnflagged) {
                    eventData.flagged_by = currentUser;
                    eventData.flagged_time = now;
                    eventData.reason = eventData.reason || 'Direct notification (bypassed flagged state)';
                }
            } else {
            ...
            writeStateEvent(eventData, function(err, result) {
                if (err) {
                    ...
                    showToast('Error updating status - retrying...');
                    // The fallback in writeStateEvent will handle retries
                }

                // === US-09 & US-10: Verbose audit logging for status changes ===
                logAction('status_changed', searchName, 'Status changed from ' + currentStatus + ' to ' + (eventData.new_status || newStatus), {
                    search_owner: owner,
                    search_app: app,
                    old_status: currentStatus,
                    new_status: eventData.new_status || newStatus,
                    reason: eventData.reason
                });
                showToast('✓ Status updated to ' + (eventData.new_status || newStatus));

                // === US-06: Auto-email on Notified ===
                // When status changes to 'notified', trigger email to owner
                if (newStatus === 'notified') {
                    var deadlineDays = CONFIG.remediationDays || 7;
                    triggerNotificationEmail(searchName, owner, app, deadlineDays);
                }

                // Update the badge in the UI optimistically
                var displayStatus = eventData.new_status || newStatus;
                var $wrapper = $('.status-dropdown-wrapper[data-search="' + searchName + '"]');
                $wrapper.data('current-status', displayStatus);
                $wrapper.find('.status-badge').parent().html(getStatusBadges(displayStatus) + '<span style="margin-left: 4px; font-size: 10px; opacity: 0.7;">▼</span>');

                // Refresh dashboard to update counts - always refresh on status changes that affect panel counts
                if (newStatus === 'resolved' || newStatus === 'notified' || newStatus === 'flagged' || newStatus === 'pending' || newStatus === 'disabled' || isSuspicious || isUnflagged) {
                    refreshDashboard();
        ...
        $(document).on('click', '#metricPopupFlag', function() {
            ...
            var remediationDays = CONFIG.remediationDays || 7;
            var now = Math.floor(Date.now() / 1000);
            var remediationDeadline = 0; // No deadline until notified (US-05)

            showToast('Flagging ' + selectedSearches.length + ' search(es)...');

            // ============================================================================
            // EVENT-SOURCING: Write state change events for each search
            // Each flag action creates an immutable event in the governance_events index
            // ============================================================================
            var completed = 0;
            var errors = 0;
            ...
            selectedSearches.forEach(function(search) {
                var eventData = {
                    ...
                    search_owner: search.owner || 'unknown',
                    search_app: search.app || 'unknown',
                    old_status: search.status || 'suspicious',
                    new_status: 'pending',
                    actor: currentUser,
                ...
                writeStateEvent(eventData, function(err, result) {
                    ...
                    // When all complete, show result
                    if (completed === selectedSearches.length) {
                        if (errors > 0) {
                            showToast('⚠ Flagged with ' + errors + ' error(s) - changes will sync within 5 minutes');
                        } else {
                        ...
                        var flaggedSearchesForExtend = selectedSearches.map(function(s) {
                            return {
                                searchName: s.name,
                                owner: s.owner,
                                app: s.app,
                        ...
                        // Offer to notify users or view in flagged modal
                        setTimeout(function() {
                            var action = confirm(
                                'Successfully flagged ' + selectedSearches.length + ' search(es).\n\n' +
                                'Note: Remediation timer starts when you notify the owner.\n\n' +
                                'Would you like to view in the Flagged modal?\n\n' +
                                'Click OK to view flagged searches.\n' +
        ...
        function updateMetricTypeButtons(metricType) {
            ...
            if (metricType === 'suspicious') {
                ...
                // Default (total): no action buttons
            }
        }
        ...
        $(document).on('click', '#metricPopupNotify', function() {
            ...
            var now = Math.floor(Date.now() / 1000);
            var remediationDays = CONFIG.remediationDays || 7;
            var deadline = now + (remediationDays * 24 * 60 * 60);

            showToast('Notifying ' + flaggedSearches.length + ' user(s)...');

            // Use writeStateEvent for each search (consistent with status dropdown)
            var completed = 0;
            var errors = 0;
            ...
            flaggedSearches.forEach(function(s) {
                ...
                writeStateEvent(eventData, function(err, result) {
                    ...
                    if (err) {
                        errors++;
                        console.error('Error notifying search:', s.name, err);
                    } else {
                        // Log action for this search
                        logAction('notified', s.name, 'User notification sent, timer started');

                        // Trigger email for this search
                        triggerNotificationEmail(s.name, s.owner, s.app, remediationDays);
                    }

                    // When all searches are processed
                    if (completed === flaggedSearches.length) {
                        if (errors > 0) {
                            showToast('⚠ Notified with ' + errors + ' error(s)');
                        } else {
                            showToast('✓ Notified ' + flaggedSearches.length + ' user(s) - Timer started (' + remediationDays + ' days)');
                        }

                        // Update UI - change status badge and show countdown
                        flaggedSearches.forEach(function(search) {
                            var $row = $('.metric-popup-row').filter(function() {
                                return $(this).find('td:eq(1)').text().trim() === search.name;
                            });
    ...
    function showSearchPreviewModal(searchName, owner, app) {
        // Decode HTML entities in case the search name was HTML-escaped
        var decodedSearchName = decodeHtmlEntities(searchName);
        var decodedOwner = decodeHtmlEntities(owner) || '-';
        var decodedApp = decodeHtmlEntities(app) || 'search';

        console.log("showSearchPreviewModal - Raw:", searchName, "Decoded:", decodedSearchName, "Owner:", decodedOwner, "App:", decodedApp);

        currentSearchPreview = { name: decodedSearchName, owner: decodedOwner, app: decodedApp, query: '' };

        $('#searchPreviewName').text(decodedSearchName || 'Unknown');
        $('#searchPreviewOwner').text(decodedOwner);
        $('#searchPreviewApp').text(decodedApp);
        ...
        // Build locale prefix from current URL
        var localePrefix = '';
        var pathParts = window.location.pathname.split('/');
        if (pathParts.length > 1 && pathParts[1].match(/^[a-z]{2}(-[A-Z]{2})?$/)) {
            localePrefix = '/' + pathParts[1];
        ...
        // Try fetching with specific owner/app first, then fall back to wildcard
        var endpoints = [];

        // First try: specific owner and app (most likely to succeed with proper permissions)
        if (decodedOwner && decodedOwner !== '-' && decodedApp && decodedApp !== '-') {
            endpoints.push(localePrefix + '/splunkd/__raw/servicesNS/' + encodeURIComponent(decodedOwner) + '/' + encodeURIComponent(decodedApp) + '/saved/searches/' + encodeURIComponent(decodedSearchName) + '?output_mode=json');
        }

        // Second try: nobody owner with specific app
        if (decodedApp && decodedApp !== '-') {
            endpoints.push(localePrefix + '/splunkd/__raw/servicesNS/nobody/' + encodeURIComponent(decodedApp) + '/saved/searches/' + encodeURIComponent(decodedSearchName) + '?output_mode=json');
        }

        // Third try: wildcard search (search across all owners and apps)
        endpoints.push(localePrefix + '/splunkd/__raw/servicesNS/-/-/saved/searches/' + encodeURIComponent(decodedSearchName) + '?output_mode=json');

        // Try endpoints in order until one succeeds
        function tryNextEndpoint(index) {
            if (index >= endpoints.length) {
                console.error('All endpoints failed for search:', decodedSearchName);
                $('#searchPreviewQuery').html('<div style="color: #dc4e41;">Search not found: ' + escapeHtml(decodedSearchName) + '<br><small style="color: rgba(255,255,255,0.5);">The search may have been deleted or you may not have permission to view it.</small></div>');
                return;
            }
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.log("Endpoint " + (index + 1) + " failed:", error, "- trying next...");
                    // Try next endpoint
                    tryNextEndpoint(index + 1);
                }
            });
    ...
    function enhanceScheduleColumns() {
        ...
        $('.dashboard-panel table, .shared-resultstable, .splunk-table').each(function() {
            ...
            $table.find('thead th').each(function(index) {
                ...
                if (text === 'Days Left') daysLeftColIndex = index;
            });
            ...
            $table.find('tbody tr').each(function() {
                ...
                // === NEW ICON LOGIC (US-02) ===
                // Priority: Disabled (🔴) > Notified (🔔) > Flagged (🚩) > Suspicious (⚡) > OK (no icon)
                // Plus 🔍 preview icon always at the end
                // RULE: Disabled overrides ALL other status icons - no combinations allowed
                ...
                if (statusColIndex >= 0 && $cells.length > statusColIndex) {
                    ...
                    if ($wrapper.length) {
                        currentStatusText = ($wrapper.data('current-status') || '').toLowerCase();
                    } else {
                        var $badge = $statusCell.find('.status-badge');
                        currentStatusText = ($badge.length ? $badge.text() : $statusCell.text()).trim().toLowerCase();
                    }

                    // Determine status flags
                    isDisabled = (currentStatusText === 'disabled' || currentStatusText === 'disabled by governance' || currentStatusText === 'auto-disabled');
                    isNotified = (currentStatusText === 'pending remediation');
                    isPendingReview = (currentStatusText === 'pending review');
                    isSuspiciousStatus = (currentStatusText === 'suspicious');
                }

                // Find the search name cell
                var $searchNameCell = null;
                if (searchName) {
                    $searchNameCell = $cells.filter(function() {
                ...
                if ($searchNameCell && $searchNameCell.length && searchName) {
                    // Remove ALL existing status indicators (clean slate)
                    $searchNameCell.find('.flag-indicator, .disabled-indicator, .suspicious-indicator, .notified-indicator, .pending-review-indicator, .ok-indicator').remove();
                    $row.removeClass('row-disabled row-suspicious row-flagged row-notified row-pending-review row-ok');

                    // Add exactly ONE status icon based on priority
                    // Priority: Disabled > Notified > Pending Review > Flagged > Suspicious > OK
                    if (isDisabled) {
                        // DISABLED overrides everything - no other icons
                        ...
                        // FLAGGED - awaiting notification
                        var flagHtml = '<span class="flag-indicator" style="color: #dc4e41; margin-right: 6px; font-size: 12px;" title="Flagged for review - awaiting notification">🚩</span>';
                        $searchNameCell.prepend(flagHtml);
                        $row.addClass('row-flagged');
                    } else if (isSuspiciousStatus) {
                        ...
                        // OK status - green checkbox
                        var okHtml = '<span class="ok-indicator" style="color: #53a051; margin-right: 6px; font-size: 12px;" title="Search is healthy">✅</span>';
                        $searchNameCell.prepend(okHtml);
                        $row.addClass('row-ok');
                    }

                    // Add magnifying glass preview icon (always present, at the end)
                ...
                // === ENHANCE DAYS LEFT COLUMN (US-03, US-08) ===
                // - Blank for OK/Suspicious/Disabled
                // - "⏸ Awaiting" styled for Flagged
                // - Countdown with 📅 extend icon for Notified
                if (daysLeftColIndex >= 0 && $cells.length > daysLeftColIndex && searchName) {
                    var $daysLeftCell = $cells.eq(daysLeftColIndex);
                    var daysLeftValue = $daysLeftCell.text().trim();

                    // Only enhance if not already enhanced
                    if (!$daysLeftCell.find('.days-left-enhanced').length) {
                        var daysLeftHtml = '';

                        if (daysLeftValue === '⏸ Awaiting' || daysLeftValue.indexOf('Awaiting') > -1) {
                            // Flagged status - show styled awaiting message
                            daysLeftHtml = '<span class="days-left-enhanced days-left-awaiting" ' +
                                'style="color: #f1813f; font-weight: 500;">' +
                                '⏸ Awaiting Notification' +
                                '</span>';
                        } else if (daysLeftValue && !isNaN(parseFloat(daysLeftValue))) {
                            // Notified status - show countdown with extend icon
                            var daysNum = parseFloat(daysLeftValue);
                            var daysColor = '#53a051';
                            var timerIcon = '🟢';
                            if (daysNum <= 1) {
                                daysColor = '#dc4e41';
                            ...
                            daysLeftHtml = '<span class="days-left-enhanced days-left-countdown" ' +
                                'data-search="' + escapeHtml(searchName) + '" ' +
                                'data-owner="' + escapeHtml(owner) + '" ' +
                                'data-app="' + escapeHtml(app) + '" ' +
                                'data-days="' + daysNum + '" ' +
                        ...
                        if (daysLeftHtml) {
                            $daysLeftCell.html(daysLeftHtml);
                        }
                    }
                }
            });
    ...
    function showExtendDeadlineModal(searchName, owner, app, currentDays) {
        ...
        $('#extendDeadlineModal').remove();

        var modalHtml = '<div id="extendDeadlineModal" style="' +
            ...
            '<h2 style="color: #00d4ff; margin: 0 0 16px 0; font-size: 18px;">📅 Extend Deadline</h2>' +
            '<p style="color: rgba(255,255,255,0.7); margin: 0 0 8px 0; font-size: 13px;">Search: <strong style="color: #fff;">' + escapeHtml(searchName) + '</strong></p>' +
            '<p style="color: rgba(255,255,255,0.7); margin: 0 0 16px 0; font-size: 13px;">Current: <strong style="color: #f8be34;">' + currentDays.toFixed(1) + ' days remaining</strong></p>' +
            '<div style="margin-bottom: 16px;">' +
            '<p style="color: rgba(255,255,255,0.6); font-size: 12px; margin: 0 0 8px 0;">QUICK EXTEND:</p>' +
            '<div style="display: flex; gap: 8px; flex-wrap: wrap;">' +
            '<button class="extend-preset-btn" data-days="3" style="' +
            'background: #2d4a6f; color: #fff; border: none; padding: 8px 16px; border-radius: 6px; ' +
            'cursor: pointer; font-weight: 600; transition: all 0.2s;">+3 Days</button>' +
            '<button class="extend-preset-btn" data-days="7" style="' +
            'background: #2d4a6f; color: #fff; border: none; padding: 8px 16px; border-radius: 6px; ' +
            'cursor: pointer; font-weight: 600; transition: all 0.2s;">+7 Days</button>' +
            '<button class="extend-preset-btn" data-days="14" style="' +
            'background: #2d4a6f; color: #fff; border: none; padding: 8px 16px; border-radius: 6px; ' +
            'cursor: pointer; font-weight: 600; transition: all 0.2s;">+14 Days</button>' +
            '<button class="extend-preset-btn" data-days="30" style="' +
            'background: #2d4a6f; color: #fff; border: none; padding: 8px 16px; border-radius: 6px; ' +
            'cursor: pointer; font-weight: 600; transition: all 0.2s;">+30 Days</button>' +
            '</div>' +
            '</div>' +
            '<div style="margin-bottom: 16px;">' +
            '<p style="color: rgba(255,255,255,0.6); font-size: 12px; margin: 0 0 8px 0;">OR CHOOSE CUSTOM DATE:</p>' +
            '<input type="date" id="extendCustomDate" style="' +
            ...
            '<button id="extendCancelBtn" style="' +
            'background: rgba(255,255,255,0.1); color: #fff; border: 1px solid rgba(255,255,255,0.2); ' +
            'padding: 10px 20px; border-radius: 6px; cursor: pointer; font-weight: 500;">Cancel</button>' +
            '<button id="extendConfirmBtn" style="' +
            'background: linear-gradient(135deg, #00d4ff 0%, #0099cc 100%); color: #000; border: none; ' +
            'padding: 10px 20px; border-radius: 6px; cursor: pointer; font-weight: 600;" disabled>Apply Custom Date</button>' +
            '</div>' +
            '</div>' +
            '</div>';
        ...
        // Store search info for handlers
        $('#extendDeadlineModal').data('search-name', searchName);
        $('#extendDeadlineModal').data('owner', owner);
        $('#extendDeadlineModal').data('app', app);

        // Set min date to today
        var today = new Date().toISOString().split('T')[0];
        $('#extendCustomDate').attr('min', today);

        // Enable confirm button when custom date is selected
        $('#extendCustomDate').on('change', function() {
            $('#extendConfirmBtn').prop('disabled', !$(this).val());
        });

        // Preset button hover effect
        $(document).on('mouseenter', '.extend-preset-btn', function() {
            $(this).css('background', '#3d5a8f');
        });
        $(document).on('mouseleave', '.extend-preset-btn', function() {
            $(this).css('background', '#2d4a6f');
        });
    }

    // Handle preset button clicks
    $(document).on('click', '.extend-preset-btn', function(e) {
        e.preventDefault();
        var extendDays = parseInt($(this).data('days'));
        var $modal = $('#extendDeadlineModal');
        var searchName = $modal.data('search-name');
        var owner = $modal.data('owner');
        var app = $modal.data('app');

        extendDeadline(searchName, owner, app, extendDays);
        $modal.remove();
    });

    // Handle custom date confirm
    $(document).on('click', '#extendConfirmBtn', function(e) {
        e.preventDefault();
        var customDate = $('#extendCustomDate').val();
        ...
        // Calculate days from now to custom date
        var now = new Date();
        var targetDate = new Date(customDate + 'T23:59:59');
        ...
        extendDeadline(searchName, owner, app, diffDays);
        $modal.remove();
    });

    // Handle cancel button
    $(document).on('click', '#extendCancelBtn', function(e) {
        e.preventDefault();
        $('#extendDeadlineModal').remove();
    });

    // Close modal on backdrop click
    $(document).on('click', '#extendDeadlineModal', function(e) {
        if (e.target === this) {
            $(this).remove();
        }
    });
    ...
    function extendDeadline(searchName, owner, app, extendDays) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // === US-10: Verbose audit logging for deadline extension ===
                logAction('deadline_extended', searchName, 'Deadline extended by ' + extendDays + ' days (new deadline: ' + newDeadlineDate + ')', {
                    search_owner: owner,
                    search_app: app,
                    new_deadline: newDeadlineDate,
                    extension_days: extendDays.toString(),
                    reason: 'Manual deadline extension via UI'
                });
                showToast('✓ Deadline extended by ' + extendDays + ' days');

                // Trigger extend email (US-07)
                triggerExtendEmail(searchName, owner, app, newDeadlineDate, extendDays);
    ...
    $(document).ready(function() {
        console.log("SA-cost-governance: Document ready, initializing...");
    ...
    console.log("SA-cost-governance: Script loaded");
