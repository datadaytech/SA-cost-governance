    ...
    function initModals() {
        ...
        $(document).on('click', '.status-option', function(e) {
            ...
            if ((isSuspicious || isUnflagged) && newStatus === 'flagged') {
                ...
                updateQuery = '| inputlookup flagged_searches_lookup ' +
                    '| eval status = if(search_name="' + escapeString(searchName) + '", "notified", status)' +
                    '| eval remediation_deadline = if(search_name="' + escapeString(searchName) + '", ' + remediationDeadline + ', remediation_deadline)' +
                    '| eval notification_sent = if(search_name="' + escapeString(searchName) + '", 1, notification_sent)' +
                    '| eval notification_time = if(search_name="' + escapeString(searchName) + '", ' + now + ', notification_time)' +
                    '| outputlookup flagged_searches_lookup';
            } else {
            ...
            runSearch(updateQuery, function(err, results) {
                if (err) {
                    ...
                    logAction('status_changed', searchName, 'Status changed from ' + currentStatus + ' to ' + newStatus, {
                        search_owner: owner,
                        search_app: app,
                    ...
                    if (newStatus === 'resolved' || newStatus === 'notified' || isSuspicious || isUnflagged) {
                        refreshDashboard();
        ...
        $('.dashboard-panel table, .shared-resultstable, .splunk-table').each(function() {
            ...
            $table.find('tbody tr').each(function() {
                ...
                if ($searchNameCell && $searchNameCell.length && searchName) {
                    ...
                    $searchNameCell.find('.flag-indicator, .disabled-indicator, .suspicious-indicator, .notified-indicator').remove();
                    $row.removeClass('row-disabled row-suspicious row-flagged row-notified');
                    ...
                    // OK status = no status icon, just 🔍 preview
